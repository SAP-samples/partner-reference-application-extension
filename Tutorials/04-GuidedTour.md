...
1. Launch your Poetry Slam Manager application directly from your subscriber subaccount.
    ...
    i. In the SAP BTP cockpit of the subscriber subaccount, navigate to **Services > Instances and Subscription**.
...
4. Now, let's add a new caterer using the **Caterer** application. This functionality is maintained separately, as a caterer is created once and can then be assigned to poetry slam events within the PSM application. Typically, this task would be performed by a manager responsible for registering and managing caterers. To get started, launch the **Caterer** application.

    i. In the SAP BTP cockpit of the subscriber subaccount, navigate to **HTML5 > HTML5 Applications**.
...
8. Choose **Edit** and select the **Caterer Name** value list. Choose **Go** to refresh the caterer list, ensuring that the newly created caterer is visible. Next, select **The Sheraton** as the caterer for your event and save your changes.
