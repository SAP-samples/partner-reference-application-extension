...
4. Add the following fields to the destination.
...
> Note: For more information on *HTTP Destinations*, refer to [SAP BTP Connectivity - HTTP Destinations](https://help.sap.com/docs/connectivity/sap-btp-connectivity-cf/http-destinations?locale=en-US).

## Add a Web Application with SAP Fiori Elements

You create the SAP Fiori application using the OData service from the **SAP Fiori Elements Application Wizard** under the *partner-reference-extension-catering-ui* folder. Open the SAP Business Application Studio from the subscriber subaccount and follow the steps below:

1. To start a new development project, open the terminal in SAP Business Application Studio and navigate to the parent directory. Then, create a new folder named *partner-reference-extension-catering-ui* by running the following command:

   ```bash
      mkdir partner-reference-extension-catering-ui
   ```

2. Go to the settings in SAP Business Application Studio and open the *Command Palette...* (Windows/Linux: Press `Ctrl + Shift + P`, Mac: Press `Cmd + Shift + P`).
3. To start the wizard, search for *SAP Business Application Studio: New Project from Template* in the *Command Palette...*.
4. Select the *SAP Fiori Generator* module template and choose *Start*.
5. Choose *List Report Page*. Choose *Next*.
6. Select the data source and the OData service as follows:
   - *Data source*: *Connect to an OData Service*
   - *OData service URL*: `https://poetry-slams-caterer.dest/odata/v4/poetryslamservice`

   > Note: In the OData service URL, `poetry-slams-caterer` is the destination name to access the base application.

7. Select the main entity from the list:
   - *Main entity*: *x_Caterers*
   - *Automatically add table columns*: *Yes*
   - *Table Type*: *Responsive*
8. Add further project attributes:
   - *Module name*: *caterer*
   - *Application title*: *Manage Caterers*
...
2. Enter the SAP BTP Cloud Foundry runtime API of your environment, for example, `https://api.cf.eu10.hana.ondemand.com`.
   ...
   4. Enter your development user and password.
...
You have already created a *Launchpad Site* in *SAP Build Work Zone* for the *Poetry Slam Manager* application. Now you have to add *Manage Caterers* web application.
...
6. Choose **HTML5 Apps**.
7. Select the **Manage Caterers** web application.
8. Choose **Add**.
...
2. Assign the **Manage Caterers** application to this role and save your changes.
...
3. Enter a title, for example **Manage Caterers**, and a description, for example **Maintain caterer** for better organization.
4. Add the **Manage Caterers** application to the newly created group.
5. Save your changes.
...
> Note: To access the *Manage Caterers* application, assign the `PoetrySlamCatererManager` role collection to your user in the subscriber subaccount. This makes the *Manage Caterers* tile visible on the site. Additionally, assign the `~poetry_slam_manager_poetrySlamManagerRole` role collection to ensure access to the underlying OData service.
